...
import CreativasShort from "../assets/CreativasShort.png"
import hamburguesaicon from "../assets/hamburguesa2.png";
import cerraricon from "../assets/cruz.png";
import busqueda from "../assets/busqueda.png"

const Navbar = ({ onSearch, searchTerm, onOpenModal, isSidebarOpen, setIsSidebarOpen }) => {
  ...
  const [showDropdown, setShowDropdown] = useState(window.innerWidth > 1115);
  const [image, setImage] = useState(window.innerWidth > 1024);
  const [isSmallScreen, setIsSmallScreen] = useState(window.innerWidth <= 1024);
  const [isSidebarSmall, setIsSidebarSmall] = useState(window.innerWidth <= 764);
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      setShowDropdown(window.innerWidth > 1115);
      setImage(window.innerWidth > 1024);
      setIsSmallScreen(window.innerWidth <= 1024);
      setIsSidebarSmall(window.innerWidth <= 764);
    };
  ...
  {image ? (
    <img src={CreativasLogo} className="ml-0 sm:ml-8 md:ml-8 lg:ml-2 xl:ml-2 h-10 w-56 md:h-12 md:w-60 lg:h-12 lg:w-64 xl:h-12 xl:w-64" alt="Creativas Short Logo" />
  ) : (
    <div className="flex items-center space-x-6">
      <img src={CreativasShort} className="h-16 w-16 ml-4" alt="Creativas Logo" />
      <img src={hamburguesaicon} alt="search icon" className="h-5 w-5 cursor-pointer" />
    </div>
  )}
           {showDropdown && (
            <Dropdown
              ...
              radius="sm"
              classNames={{
                base: "before:bg-default-200 ml-2",
            ...
            >
              <DropdownTrigger>
                <Button
                  variant='solid'
                  color='success'
                  className='text-white hover:bg-secondary-ligth z-0'
                  radius="full"
                  startContent={<img src={explorerIcon} alt='explorer' className='w-4 h-4'></img>}
                >
                  Explorer
                </Button>
              </DropdownTrigger>
              ...
              >
                <DropdownSection aria-label="Actions" showDivider classNames={{ divider: "bg-white" }}>
                  <DropdownItem key="dashboard" startContent={<img src={cubo} className='h-4 w-4' alt='cubo'></img>}>
                    All projects
                  </DropdownItem>
                  <DropdownItem key="settings" startContent={<img src={nfts} className='h-4 w-4' alt='nfts'></img>}>
                    Marketplace
                  </DropdownItem>
                  <DropdownItem key="new_project" onClick={onOpenModal} startContent={<img src={masicon} className='h-4 w-4 ' alt='masicon'></img>}>
                    New Project
                  </DropdownItem>
                </DropdownSection>

                <DropdownSection aria-label="Preferences" showDivider classNames={{ divider: "bg-white" }}>
        ...
        {!isSmallScreen ? (
        <div className="flex-grow flex items-center justify-center ml-4 mt-4 md:ml-4 lg:ml-4 xl:ml-6">
          <Input
            color='default'
            variant='bordered'
            radius='md'
  ...
  <ul className="font-thin flex flex-row space-x-2 rtl:space-x-reverse">
  <li>
    </li>
    <li>
      <div className='mt-3'>
        <img src={busqueda} alt="search icon" className="h-5 w-5 cursor-pointer" />
      </div>
    </li>
    <li>
      <div className='-mt-0.5'>
              ...
              <Chain isSmallScreen={isSmallScreen}/>
      ...
      {isLoggedIn ? (
                <div className="relative">
                  ...
                  <button
                    ...
                    className="text-white  bg-customblack border border-green/5 hover:bg-gray-800 rounded-te focus:outline-none font-thin rounded-full text-sm flex items-center space-x-1"
                  >
                  ...
                  {isSidebarOpen && (
                    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-end">
                      <div className="relative">
                        <Sidebar onClose={closeSidebar} isSidebarOpen={isSidebarOpen} setIsSidebarOpen={setIsSidebarOpen} onOpenModal={onOpenModal}   isSmallScreen={isSidebarSmall}/>
                      </div>
                    </div>
                  )}
                </div>
              ...
              {!isSmallScreen && (
        <div className="w-full md:w-auto mr-6 mt-4" id="navbar-default">
      {isWideScreen ? (
        <ul className="font-thin flex flex-row md:p-0 md:space-x-4 rtl:space-x-reverse md:mt-0 md:border-0">
          <li>
            <a href="#" className="block px-2 text-base text-white rounded md:bg-transparent hover:text-white">NFT Venture</a>
          </li>
          <li>
            <a href="#" className="block px-2 text-base text-[#9398A7] rounded md:bg-transparent hover:text-white">Marketplace</a>
          </li>
          <li>
            <a href="#" className="block px-2 text-base text-[#9398A7] rounded md:bg-transparent hover:text-white" aria-current="page">Portfolio</a>
          </li>
        </ul>
      ) : (
        <button className="px-2  text-base text-white rounded bg-transparent hover:text-white">
          <img src={hamburguesaicon} className="h-4 w-4" alt="Menu Icon" />
        </button>
      )}
    </div>
       )}
...
{!isSmallScreen && (
        <div className="w-full md:w-auto mt-4" id="navbar-default">
              ...
              {isLoggedIn ? (
                <div className="relative">
                  ...
                  {isSidebarOpen && (
                    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-end">
                      <div className="relative">
                        <Sidebar onClose={closeSidebar} isSidebarOpen={isSidebarOpen} setIsSidebarOpen={setIsSidebarOpen} onOpenModal={onOpenModal}  isSmallScreen={isSmallScreen}/>
                      </div>
                    </div>
                  )}
                </div>
